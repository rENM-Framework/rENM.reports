...
#'
#' Builds a single-page PDF combining a suitability trend map with centroids,
#' a centroid trend summary table, and a caption into a structured layout.
#'
#' @details
#' \strong{Pipeline context}
#' This function composes a precomputed centroid trend map, a summary table,
#' and a package caption into a single standardized reporting page. It operates
#' entirely within the rENM project directory and does not rely on external
#' scripts.
#'
#' \strong{Inputs}
...
#' \preformatted{
#'   inst/captions/centroid_shift_caption.pdf
#' }
#'
#' \strong{Outputs}
...
#' \itemize{
#'   \item The centroid trend map is placed at the top of the content area,
#'         scaled to fill the content width (subject to \code{map_width_factor})
#'         while preserving its aspect ratio.
#'   \item A summary table is placed \code{v_gap_in} inches below the map,
...
#' The caption PDF (\code{centroid_shift_caption.pdf}) is rasterized via
...
#'         \code{trim_map}) and the table (controlled by \code{trim_table})
...
#' @param v_gap_in Numeric. Vertical gap between map and table in inches.
#' Default 0.30.
...
assemble_centroid_trends_page <- function(alpha_code,
  ...
  in_map_png  <- file.path(runs_dir, "Trends", "suitability",
                            sprintf("%s-Suitability-Trend-With-Centroids.png", code))
  in_tbl_pdf  <- file.path(runs_dir, "Summaries", "tables",
                            sprintf("%s-Centroid-Trend-Summary.pdf", code))
  in_caption_pdf <- system.file("captions", "centroid_shift_caption.pdf",
                                package = "rENM.reports")
  out_pdf <- file.path(pages_dir, sprintf("%s-Centroid-Trends.pdf", code))

  # ---- Validate inputs --------------------------------------------------------
  for (f in c(in_map_png, in_tbl_pdf)) {
    if (!file.exists(f)) {
      stop("Missing input file: ", f, call. = FALSE)
    }
  }
  if (!nzchar(in_caption_pdf) || !file.exists(in_caption_pdf)) {
    stop("Caption PDF not found in installed package: inst/captions/centroid_shift_caption.pdf",
         call. = FALSE)
  }
  ...
  tbl_png <- pdftools::pdf_convert(
    ...
    dpi       = table_pdf_dpi,
    pages     = 1L,
    filenames = file.path(tempdir(), "centroid_table_1.png")
  )
  ...
  cap_png <- pdftools::pdf_convert(
    ...
    pages     = 1L,
    filenames = file.path(tempdir(), "centroid_shift_caption_1.png")
  )
  ...
  max_map_h_avail <- content_h - vG - tbl_h - vCG - cap_h
  ...
  # ---- Composite (centered map, table, and caption) ---------------------------
