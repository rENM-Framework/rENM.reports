...
#' \code{<year>/model/}
#' \code{<alpha_code>-<year>-Prediction.png}
#'
...
#' \itemize{
#'   \item A one-page PNG (<alpha_code>-Suitability-Maps.png)
#'   \item A one-page PDF (<alpha_code>-Suitability-Maps.pdf)
#'   \item A one-page DOCX (<alpha_code>-Suitability-Maps.docx)
#' }
#'
...
#' montage so it sits 1 inch from the top margin and centered horizontally.
#'
#' \strong{Data requirements}
#' Exactly 9 prediction PNG files must exist for the specified alpha_code.
#'
...
#'
#' @export
gather_suitability_maps <- function(alpha_code,
                                    margin_in = 1,
                                    page_width_in = 8.5,
                                    page_height_in = 11,
                                    tile = "3x3",
  ...
  if (length(missing) > 0) {
    stop(sprintf(
      "Missing %d expected prediction map(s) for '%s':\n%s",
  ...
  # Outputs
  base_name <- file.path(out_dir, sprintf("%s-Suitability-Maps", code))
  ...
  header <- "Processing summary (gather_suitability_maps)"
